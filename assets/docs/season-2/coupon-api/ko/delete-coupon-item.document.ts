export const  deleteCouponItemDocument = {
  note: ``,

  //don't modify documentInfo
  documentInfo: {
    category: `coupon-api`,   // input category ex) exchange-service-api
    id: `delete-coupon-item`,           // use **dash** and *english*  ex) coupang-confirm-request-creation
    anchorId: `delete_coupon_item.`,
    name: `쿠폰아이템 파기`,       // use display name, i will change 'translation key'
  ...
  apiInfo: {
    ...
    httpMethod: `PUT`,
    path: `/v2/providers/fms/apis/api/v1/vendors/{vendorId}/coupons/{couponId}/items?action=expire`,
    HMACPath: `/v2/providers/fms/apis/api/v1/vendors/{vendorId}/coupons/{couponId}/items?action=expire`,
    _description: `아이템에 적용되어 있던 쿠폰의 적용 해제를 위한 API입니다. 본 API는 비동기방식으로 프로세스에 대한 요청 만을 수행하고 있어
<br/>실제 API 수행 결과값은 응답으로 받은 "requestedId" 값을 가지고 요청상태 확인 API를 통해서 확인 가능합니다.`,
    ...
    bodyParameters: [
      {
        ...
        type: `Array`,
        require: false,
        _description: `쿠폰 적용을 해제할 아이템 ID`,
        _relation: ``,
        _referenceInfo: `쿠폰 적용을 해제할 아이템 ID 수는 10,000개를 초과할 수 없음`,
        _warning: ``,
        children: false
      }
    ]
  ...
  sample: {
    endpoint: `https://api-gateway.coupang.com/v2/providers/fms/apis/api/v1/vendors/A000000001/coupons/84/items?action=expire`,
    code: [
      {
        language: `http`,
        codeblock: {
